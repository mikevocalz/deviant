...
export default function SearchScreen() {
  ...
  const { searchQuery: storeQuery, setSearchQuery, clearSearch } = useSearchStore()
  const [searchQuery, setLocalQuery] = useState(params.query || storeQuery || "")
  ...
  const { data: postsData, isLoading: isLoadingPosts } = useSearchPosts(searchQuery)
    ...
    <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
      ...
      <ScrollView className="flex-1">
        {isLoading ? (
          ...
          <View className="p-4">
            <Text className="text-base font-semibold mb-3 text-foreground">Search for posts, users, or hashtags</Text>
            ...
            {isHashtag ? (
              <>
                <View className="p-4 border-b border-border">
                  <View className="flex-row items-center gap-2">
                    ...
                    <Text className="text-lg font-semibold text-foreground">{searchQuery}</Text>
                  </View>
              ...
              <>
                {userResults.length > 0 && (
                  <View className="p-4 border-b border-border">
                    <Text className="text-base font-semibold text-foreground mb-3">Users</Text>
                    {userResults.map((user: any) => (
                      ...
                      >
                        <Image 
                          source={{ 
                            uri: user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.username || "User")}`
                          }} 
                ...
                {searchResults.length > 0 && (
                  <View className="p-4">
                    <Text className="text-base font-semibold text-foreground mb-3">Posts</Text>
                    <View className="flex-row flex-wrap">
                      {searchResults.map((post: any) => {
                        const firstMedia = post.media?.[0];
                        const thumbnail = firstMedia?.url;
