...
export const sneakyLynkApi = {
   ...
   */
  async createRoom(params: CreateRoomParams): Promise<ApiResponse<{ room: SneakyRoom }>> {
    return callEdgeFunction("sneaky_create_room", {
      title: params.title,
      topic: params.topic,
  ...
  async joinRoom(roomId: string): Promise<ApiResponse<JoinRoomResponse>> {
    return callEdgeFunction("sneaky_join_room", { roomId });
  },

  /**
   * Refresh Fishjam token
   */
  async refreshToken(roomId: string): Promise<ApiResponse<{ token: string; expiresAt: string }>> {
    return callEdgeFunction("sneaky_refresh_token", { roomId });
  },

  /**
   * Kick a user from the room (host/moderator only)
   */
  async kickUser(roomId: string, targetUserId: string, reason?: string): Promise<ApiResponse<void>> {
    return callEdgeFunction("sneaky_kick_user", { roomId, targetUserId, reason });
  },

  /**
   * Ban a user from the room (host/moderator only)
   */
  async banUser(roomId: string, targetUserId: string, reason?: string): Promise<ApiResponse<void>> {
    return callEdgeFunction("sneaky_ban_user", { roomId, targetUserId, reason });
  },
  ...
  async endRoom(roomId: string): Promise<ApiResponse<void>> {
    return callEdgeFunction("sneaky_end_room", { roomId });
  },

  /**
   * Raise/lower hand
   */
  async toggleHand(roomId: string, raised: boolean): Promise<ApiResponse<void>> {
    return callEdgeFunction("sneaky_toggle_hand", { roomId, raised });
  },
