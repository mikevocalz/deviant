...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!postId) return errorResponse("validation_error", "postId is required", 400);
    ...
    if (!post || post.author_id !== userData.id) return errorResponse("forbidden", "You can only update your own posts", 403);

    const updateData: any = {};
    if (content !== undefined) updateData.content = content;
    if (location !== undefined) updateData.location = location;

    const { data: updated, error } = await supabaseAdmin.from("posts").update(updateData).eq("id", postId).select().single();
    if (error) return errorResponse("internal_error", "Failed to update post", 500);
