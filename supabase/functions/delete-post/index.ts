...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!post || post.author_id !== userData.id) return errorResponse("forbidden", "You can only delete your own posts", 403);
    ...
    if (error) return errorResponse("internal_error", "Failed to delete post", 500);
