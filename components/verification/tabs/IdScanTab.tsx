...
export default function IdScanTab() {
  ...
  useEffect(() => {
    ...
    persistVerificationPhoto(imageUri, `id-${Date.now()}.jpg`)
      .then((saved) => {
        setIdImageUri(saved)
        setIdComplete(true)
        if (dobResult.dateOfBirth) {
  ...
  }, [mode, imageUri, scanProgress, busy, ocrText, setParsedId, setIdImageUri, setIdComplete])
  ...
  if (idComplete && storedImageUri) {
    return (
      <View className="flex-1 bg-background rounded-2xl overflow-hidden" style={{ minHeight: 300 }}>
        <Image source={{ uri: storedImageUri }} className="flex-1" resizeMode="contain" />
        ...
        <View className="absolute bottom-4 left-0 right-0 items-center">
          <View className="bg-primary/90 px-4 py-2 rounded-full">
            <Text className="text-primary-foreground font-medium">ID Captured ✓</Text>
          </View>
        </View>
      </View>
    )
  ...
  if (mode === 'select') {
    return (
      <View className="flex-1 bg-card rounded-2xl items-center justify-center gap-6 p-6" style={{ minHeight: 300 }}>
        <View className="bg-muted/30 rounded-full p-6">
          <CreditCard size={48} className="text-muted-foreground" />
        </View>
        
        <View className="items-center gap-2">
          <Text className="text-lg font-semibold text-foreground">Upload ID Document</Text>
          <Text className="text-sm text-muted text-center">
            Take a photo or choose an existing image of your government-issued ID
          </Text>
        </View>
        ...
        <View className="w-full gap-3">
          ...
          >
            ...
            <Text className="text-primary-foreground font-medium">Take Photo</Text>
          </Button>
          ...
          >
            ...
            <Text className="text-foreground font-medium">Choose from Library</Text>
          </Button>
        </View>
      </View>
    )
    ...
    if (!device) {
      return (
        <View className="flex-1 bg-card rounded-2xl items-center justify-center" style={{ minHeight: 300 }}>
          ...
          <Button variant="outline" onPress={() => setMode('select')} className="mt-4">
            Go Back
          </Button>
        </View>
      )
        ...
        <View className="absolute bottom-6 left-0 right-0 px-6 gap-3">
          ...
          <View className="flex-row gap-3">
            ...
            <Button 
              ...
              className="flex-1"
            >
              <Text className="text-primary-foreground">{busy ? 'Capturing...' : 'Capture'}</Text>
            </Button>
          </View>
        </View>
      </View>
  ...
  if (mode === 'scanning' && imageUri) {
    ...
    return (
      <View className="flex-1 bg-card rounded-2xl items-center justify-center gap-6 p-6" style={{ minHeight: 300 }}>
        ...
        <Text className="text-lg font-semibold text-foreground">Scanning ID Document...</Text>

        {/* Progress bar */}
        <View className="w-full gap-2">
          <Progress value={progressValue} className="h-2" />
          <Text className="text-center text-muted text-sm">{progressValue}% complete</Text>
        </View>
