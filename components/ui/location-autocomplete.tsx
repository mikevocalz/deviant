...
export function LocationAutocomplete({
  ...
  useEffect(() => {
    console.log(
      "[LocationAutocomplete] Checking API key configuration...",
    );
    if (
      ...
      console.warn(
        "[LocationAutocomplete] Google Places API key not configured or using placeholder.",
      );
      setHasError(true);
    } else {
      console.log(
        "[LocationAutocomplete] API key configured successfully",
      );
    }
  }, []);
  ...
  const loadRecentLocations = () => {
    try {
      ...
      console.error(
        "[LocationAutocomplete] Failed to load recent locations:",
        error,
      );
    }
  };
  ...
  const saveToRecentLocations = (location: LocationData) => {
    try {
      ...
      console.error(
        "[LocationAutocomplete] Failed to save recent location:",
        error,
      );
    }
  };
  ...
  const requestCurrentLocation = async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        console.log(
          "[LocationAutocomplete] Location permission denied",
        );
        return;
      }
      ...
      console.error(
        "[LocationAutocomplete] Failed to get current location:",
        error,
      );
    } finally {
  ...
  useEffect(() => {
    console.log("[LocationAutocomplete] Dropdown state changed:", {
      showDropdown,
      activeSection,
  ...
  useEffect(() => {
    console.log(
      "[LocationAutocomplete] Text changed:",
      inputText,
      "length:",
      inputText.length,
      "justSelected:",
    ...
    if (justSelectedRef.current) {
      console.log(
        "[LocationAutocomplete] Just selected, skipping fetch",
      );
      return;
    }
  ...
  const fetchPredictions = async (text: string) => {
    console.log(
      "[LocationAutocomplete] Fetching predictions for:",
      text,
    );
    setIsLoading(true);
    ...
    try {
      ...
      console.log(
        "[LocationAutocomplete] Fetching URL (NEW API):",
        url,
      );
      ...
      if (data.error) {
        console.error(
          "[LocationAutocomplete] API error (NEW):",
          data.error,
        );
        return await tryLegacyAPI(text);
      }
      ...
      console.log(
        "[LocationAutocomplete] Predictions set (NEW):",
        convertedPredictions.length,
      );
    } catch (error) {
      console.error(
        "[LocationAutocomplete] Fetch error (NEW):",
        error,
      );
  ...
  const tryLegacyAPI = async (text: string) => {
    try {
      console.log("[LocationAutocomplete] Trying legacy API");
      ...
      console.log(
        "[LocationAutocomplete] Legacy API predictions:",
        data.predictions?.length || 0,
      );
    } catch (error) {
      console.error("[LocationAutocomplete] Legacy API error:", error);
      setPredictions(getPopularLocations(text));
    }
  };
  ...
  const fetchPlaceDetails = async (placeId: string) => {
    try {
      ...
      if (data.status === "REQUEST_DENIED") {
        console.error(
          "[LocationAutocomplete] Details API error:",
          data.error_message,
        );
        return null;
      }
      ...
      console.error(
        "[LocationAutocomplete] Details fetch error:",
        error,
      );
      return null;
    }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <TextInput
          ...
          onFocus={() => {
            console.log(
              "[LocationAutocomplete] Input focused, justSelected:",
              justSelectedRef.current,
              "inputText:",
              inputText,
            );
            ...
            if (!justSelectedRef.current) {
              console.log(
                "[LocationAutocomplete] Showing dropdown on focus",
              );
              setShowDropdown(true);
              setActiveSection(
              ...
              console.log(
                "[LocationAutocomplete] Skipping dropdown - just selected",
              );
            }
          }}
          onBlur={() => {
            console.log("[LocationAutocomplete] Input blurred");
