...
const APP_SCHEME = 'deviant';
const WEB_BASE_URL = 'https://deviant.app';
...
export function generateDeepLink(content: ShareableContent): string {
  ...
  switch (type) {
    ...
    case 'profile':
      return `${WEB_BASE_URL}/profile/${id}`;
    case 'event':
      return `${WEB_BASE_URL}/events/${id}`;
    case 'story':
...
export function generateAppDeepLink(content: ShareableContent): string {
  ...
  switch (type) {
    case 'post':
      return `${APP_SCHEME}://post/${id}`;
    case 'profile':
      return `${APP_SCHEME}://profile/${id}`;
    case 'event':
      return `${APP_SCHEME}://events/${id}`;
    case 'story':
...
export async function shareContent(content: ShareableContent): Promise<void> {
  ...
  try {
    ...
    if (errorMessage !== 'Share was dismissed' && errorMessage !== 'User did not share') {
...
export async function sharePost(postId: string, caption?: string): Promise<void> {
  await shareContent({
    type: 'post',
    id: postId,
    title: 'Share Post',
    message: caption ? `${caption.substring(0, 100)}...` : 'Check out this post!',
  });
}

export async function shareProfile(username: string, displayName?: string): Promise<void> {
  await shareContent({
    type: 'profile',
    id: username,
    title: displayName ? `${displayName}'s Profile` : 'Share Profile',
    message: displayName ? `Check out ${displayName}'s profile!` : 'Check out this profile!',
  });
}
...
export async function shareStory(storyId: string, username?: string): Promise<void> {
  await shareContent({
    type: 'story',
    id: storyId,
    title: 'Share Story',
    message: username ? `Check out ${username}'s story!` : 'Check out this story!',
  });
}
