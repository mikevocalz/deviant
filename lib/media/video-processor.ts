...
import { MediaConstraints, ProcessedMedia, ValidationError, VideoInfo } from './types';
...
export async function validateVideo(
  ...
  try {
    ...
    // Check duration
    if (constraints.maxDurationSeconds && info.duration > constraints.maxDurationSeconds) {
      errors.push({
        field: 'duration',
    ...
    if (bitrateKbps > 8000) {
      console.warn('[VideoValidator] High bitrate detected, may fail on slow networks');
    }
...
export async function processVideo(
  ...
  try {
    const { uri: thumbnailUri } = await VideoThumbnails.getThumbnailAsync(sourceUri, {
