...
import { blocks } from "@/lib/api-client";
...
interface BlockResponse {
  ...
  blocked: string | { id: string; username?: string; firstName?: string; lastName?: string; avatar?: { url?: string } | string };
  reason?: string;
  createdAt: string;
}
...
function transformBlockedUser(block: BlockResponse): BlockedUser {
  ...
  const avatarUrl = typeof blocked.avatar === "object" && blocked.avatar?.url
    ? blocked.avatar.url
  ...
  return {
    ...
    name: [blocked.firstName, blocked.lastName].filter(Boolean).join(" ") || blocked.username || "Unknown",
    avatar: avatarUrl,
    blockedAt: block.createdAt,
  };
}
...
export function useBlockedUsers() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await blocks.getBlocked<BlockResponse>({ limit: 100 });
      return response.docs.map(transformBlockedUser);
    },
    enabled: !!user?.id,
...
export function useBlockUser() {
  ...
  return useMutation({
    mutationFn: async ({ userId, reason }: { userId: string; reason?: string }) => {
      return await blocks.block<BlockResponse>(userId, reason);
    },
    onMutate: async ({ userId }) => {
      ...
      const previousBlocked = queryClient.getQueryData<BlockedUser[]>(["blocked-users", user?.id]);

      // Optimistically add the blocked user (we'll get full data on success)
      // For now, add a placeholder
      if (previousBlocked) {
        queryClient.setQueryData<BlockedUser[]>(["blocked-users", user?.id], [
          ...previousBlocked,
          {
            id: `temp-${userId}`,
            blockId: `temp-${userId}`,
    ...
    onError: (error: any, variables, context) => {
      ...
      if (context?.previousBlocked) {
        queryClient.setQueryData(["blocked-users", user?.id], context.previousBlocked);
      }
...
export function useUnblockUser() {
  ...
  return useMutation({
    mutationFn: async (blockId: string) => {
      return await blocks.unblock(blockId);
    },
...
export function useIsUserBlocked(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await blocks.isBlocked(userId);
      return response;
    },
