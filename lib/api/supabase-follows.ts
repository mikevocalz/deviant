...
export const followsApi = {
  ...
  async toggleFollow(targetUserId: string, isFollowing: boolean) {
    try {
      console.log('[Follows] toggleFollow:', targetUserId, 'isFollowing:', isFollowing);
      ...
      if (isFollowing) {
        ...
        // Decrement counts
        await supabase.rpc('decrement_following_count', { user_id: userData[DB.users.id] });
        await supabase.rpc('decrement_followers_count', { user_id: parseInt(targetUserId) });
      } else {
        // Follow
        await supabase
          .from(DB.follows.table)
        ...
        // Increment counts
        await supabase.rpc('increment_following_count', { user_id: userData[DB.users.id] });
        await supabase.rpc('increment_followers_count', { user_id: parseInt(targetUserId) });
      }
