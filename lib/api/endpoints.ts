...
export const AUTH = {
  ...
  REGISTER: "/api/register", // POST - Public registration endpoint
  REGISTER_PAYLOAD: "/api/users", // POST - Create user (direct Payload, requires auth)
...
export const USERS = {
  ...
  BY_USERNAME: (username: string) =>
    `/api/users?where[username][equals]=${username}`,
} as const;
